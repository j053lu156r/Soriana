...
if (sheetobj[posicion]==undefined){
    XLSXP.utils.sheet_add_aoa(sheetobj, [[{t:tipo,v:valor}]], {origin: posicion});
}else{
...
let workbook = XLSXP.read(archivo_plantilla, {type: 'base64',cellStyles: true,cellHTML: true,cellNF:true,cellText:true});
...
newWorkBook.Sheets[SheetName].M2 = { t: 'n', v: nodo_datos.results[0].IBukrs!="5003"?0:nodo_datos.results[0].ETXTTOTALNAV.results[0].Columna13,z:formato_cantidades }; //Imp_Cargo_17R
...
for (var row of nodo_datos.results[0].ETXTFACTPROVNEWNAV.results){
    ...
    createCell(newWorkBook.Sheets[SheetName],"U"+indice_renglon,"n",row.C25TasaocuotaDr03.trim());
...
if (row.C23ImporteDr03!=""){
    suma_importe_retencion+=parseFloat(row.C23ImporteDr03);
}
...
createCell(newWorkBook.Sheets[SheetName],"L2","n",retencion_aplicada*-1);
newWorkBook.Sheets[SheetName]["L2"].z=formato_cantidades;

/* genera archivo XLSX */
XLSXP.writeFile(newWorkBook, nodo_datos.results[0].IBukrs+"_"+nodo_datos.results[0].IGjahr+"_"+nodo_datos.results[0].IVblnr+".xlsx",{codepage: 65001});
}